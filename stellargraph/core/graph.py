...
class StellarGraph:
    ...
    def _edge_weights(self, source_node: Any, target_node: Any) -> List[Any]:
        ...
        if self.is_directed():
            ...
            actual_positions = self._edges.targets[all_ilocs] == target_node
        else:
            all_ilocs = self._edges.edge_ilocs(source_node, ins=True, outs=True)

            sources = self._edges.sources[all_ilocs]
            targets = self._edges.targets[all_ilocs]

            forward_bools = (sources == source_node) & (targets == target_node)
            backward_bools = (sources == target_node) & (targets == source_node)
            actual_positions = forward_bools | backward_bools

        filtered_ilocs = all_ilocs[actual_positions]
        return [float(x) for x in self._edges.weights[filtered_ilocs]]
